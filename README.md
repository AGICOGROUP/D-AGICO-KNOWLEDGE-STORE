...
浏览器打开服务根地址（本机 `http://127.0.0.1:8765/`），输入成员访问码即可提交和查找文件。先批量选择文件，再显式选择本批所属事业部，不自动代选；资料类型与型号可不填，提交后进入待审核区。页面支持处理状态、已发布资料搜索、事业部／类型筛选和原件下载。详见[网页使用与验证](docs/validation/employee-portal.md)。
